...
fezHatWatcher::fezHatWatcher(AllJoynBusAttachment^ busAttachment) :
...
fezHatWatcher::~fezHatWatcher()
{
    UnregisterFromBus();
}

void fezHatWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.ianlee.fezHat" };
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.ianlee.fezHat"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.ianlee.fezHat"),
            port);
        Added(this, args);
    }
...
void fezHatWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<fezHatWatcher>
        };
...
void fezHatWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_fezHatIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &fezHatWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void fezHatWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void fezHatWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
