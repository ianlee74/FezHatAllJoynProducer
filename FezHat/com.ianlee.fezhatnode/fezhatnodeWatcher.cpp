...
fezhatnodeWatcher::fezhatnodeWatcher(AllJoynBusAttachment^ busAttachment) :
...
fezhatnodeWatcher::~fezhatnodeWatcher()
{
    UnregisterFromBus();
}

void fezhatnodeWatcher::UnregisterFromBus()
{
    if (nullptr != m_aboutListener)
    {
        PCSTR interfaces[] = { "com.ianlee.fezhatnode" };
...
void fezhatnodeWatcher::OnAnnounce(
    _In_ PCSTR name,
    ...
    _In_ const alljoyn_msgarg aboutDataArg)
{
    ...
    if (alljoyn_aboutobjectdescription_hasinterface(objectDescription, "com.ianlee.fezhatnode"))
    {
        AllJoynServiceInfo^ args = ref new AllJoynServiceInfo(
            AllJoynHelpers::MultibyteToPlatformString(name),
            AllJoynHelpers::GetObjectPath(objectDescription, "com.ianlee.fezhatnode"),
            port);
...
void fezhatnodeWatcher::BusAttachmentStateChanged(_In_ AllJoynBusAttachment^ sender, _In_ AllJoynBusAttachmentStateChangedEventArgs^ args)
{
    if (args->State == AllJoynBusAttachmentState::Connected)
    {
        alljoyn_aboutlistener_callback callbacks = 
        {
            AllJoynHelpers::AnnounceHandler<fezhatnodeWatcher>
        };
...
void fezhatnodeWatcher::Start()
{
    ...
    int32 result = AllJoynHelpers::CreateInterfaces(m_busAttachment, c_fezhatnodeIntrospectionXml);
    ...
    m_busAttachmentStateChangedToken = m_busAttachment->StateChanged += ref new TypedEventHandler<AllJoynBusAttachment^, AllJoynBusAttachmentStateChangedEventArgs^>(this, &fezhatnodeWatcher::BusAttachmentStateChanged);
    m_busAttachment->Connect();
}

void fezhatnodeWatcher::Stop()
{
    StopInternal(AllJoynStatus::Ok);
}

void fezhatnodeWatcher::StopInternal(int32 status)
{
    UnregisterFromBus();
    AllJoynHelpers::DispatchEvent([=]() {
